...
async def test_data():
    async with TestSessionLocal() as session:
        # Сначала создаем экзамены
        exam1 = Exam(uuid=UUID("236e43f1-6d9a-42d2-bf80-514e7ed3030c"),
                    name="Русский язык",
                    code="rus")
        exam2 = Exam(uuid=UUID("bde589f5-c13e-4606-ad55-c394038091b8"),
                    name="Математика (базовая)",
                    code="math_basic")
        session.add_all([exam1, exam2])
        await session.commit()

        # Затем создаем типы факультетов
        ...
        # Теперь можно создавать требования факультета к экзаменам
